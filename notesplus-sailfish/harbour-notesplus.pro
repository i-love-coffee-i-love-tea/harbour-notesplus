...
isEmpty(RUST_CORE_LIB) {
    RUST_CORE_LIB = $$OUT_PWD/../target/release/libnotesplus_core.a
}
!exists($$RUST_CORE_LIB) {
    # Cross-compilation target (sfdk)
    RUST_CORE_LIB = $$PWD/../target/aarch64-unknown-linux-gnu/release/libnotesplus_core.a
}
!exists($$RUST_CORE_LIB) {
    RUST_CORE_LIB = $$PWD/../target/release/libnotesplus_core.a
}
exists($$RUST_CORE_LIB) {
    PRE_TARGETDEPS += $$RUST_CORE_LIB
    LIBS += $$RUST_CORE_LIB
} else {
    error("Cannot find libnotesplus_core.a — build notesplus-core first with: cargo build --release -p notesplus-core")
}
