...
use notesplus_core::db::open_db;
use notesplus_core::page::{save_and_index_page, get_page};
use notesplus_core::parser::parse_preview_blocks;
use notesplus_core::block::Block;
...
fn test_rebuild_index_with_existing_pages_and_content_search() {
    ...
    let stats = notesplus_core::page::rebuild_index(&conn, &notes_dir)
    ...
    let search_results = notesplus_core::search::search_pages(&conn, "wolpertinger").unwrap();
