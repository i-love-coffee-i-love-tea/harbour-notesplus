...
pub struct FishdocBridge {
    ...
    auth_config: fishdoc_core::server::auth::AuthConfig,
...
impl Default for FishdocBridge {
    fn default() -> Self {
        ...
        Self {
            ...
            auth_config: fishdoc_core::server::auth::AuthConfig::default(),
            conn_receiver: None,
        }
    }
}
        ...
        std::thread::spawn(move || {
            ...
            match rusqlite::Connection::open(&db_path) {
                Ok(conn) => {
                    ...
                    let t = std::time::Instant::now();
                    let _ = page::copy_examples(&conn, &notes_path, std::path::Path::new("/usr/share/harbour-fishdoc/examples"));
