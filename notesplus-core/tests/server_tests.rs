use std::fs;
use std::sync::Arc;
use notesplus_core::server::*;
use notesplus_core::agent::client::LlmConfig;
use notesplus_core::agent::permissions::PermissionConfig;
use serde_json::json;
...
fn test_note_filename_sanitization() {
    ...
    assert_eq!(notesplus_core::page::sanitize_note_filename("  "), "Untitled.adoc");
    assert_eq!(notesplus_core::page::sanitize_note_filename("Hello World 123"), "Hello_World_123.adoc");
}
...
fn test_link_page_widget_web_assets() {
    use notesplus_core::server::web_assets::{INDEX_HTML, APP_JS, STYLE_CSS, STORES_LINK_STORE_JS};
...
fn test_ai_model_selection_web_assets() {
    use notesplus_core::server::web_assets::{INDEX_HTML, APP_JS, STYLE_CSS, STORES_AI_JS};
...
fn test_gallery_view_and_rendered_cards_web_assets() {
    use notesplus_core::server::web_assets::{INDEX_HTML, STYLE_CSS, STORES_NOTES_JS};
...
fn test_web_ui_logout_button_assets() {
    use notesplus_core::server::web_assets::{INDEX_HTML, APP_JS, STYLE_CSS, STORES_AUTH_JS};
...
fn test_web_ui_responsive_header_and_account_dropdown_assets() {
    use notesplus_core::server::web_assets::{INDEX_HTML, APP_JS, STYLE_CSS};
...
fn test_import_from_url_and_file_web_assets() {
    use notesplus_core::server::web_assets::{INDEX_HTML, APP_JS, STYLE_CSS, STORES_IMPORT_STORE_JS};
...
fn test_theme_api_endpoint() {
    ...
    let qt_theme = notesplus_core::html::qt_html::QtThemeColors::from_map(&colors);
...
fn test_server_start_with_theme_config() {
    ...
    let handle = notesplus_core::ffi::notes_core_server_start(
        c_notes_dir.as_ptr(),
        c_db_path.as_ptr(),
        c_backup_dir.as_ptr(),
        0,
    ...
    notesplus_core::ffi::notes_core_server_stop(handle);
}
...
fn test_contextual_navbar_and_blocks_mode_removal() {
    use notesplus_core::server::web_assets::{
...
fn test_rebuild_index_and_search_chronicles_wolpertinger() {
    ...
    let conn = notesplus_core::db::open_db(&db_path).unwrap();
    let stats = notesplus_core::page::rebuild_index(&conn, &notes_dir).expect("rebuild_index should succeed");
...
fn test_note_color_selection_and_persistence() {
    ...
    assert!(notesplus_core::page::NOTE_CARD_PALETTE.contains(&reset_color));
